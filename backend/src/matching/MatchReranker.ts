import type { MatchDecision, ParsedDocument, SimilaritySignals } from './types.js'

export interface IMatchReranker {
  rerankIfNeeded(params: {
...
export class MatchReranker implements IMatchReranker {
...
candidateChunks=${JSON.stringify(candidateTopChunks)}`
    ...
    if (!resp || !resp.ok) return params.initialDecision
    ...
    if (!content) return params.initialDecision
    const cleaned = content.replace(/```(?:json)?/g, '').replace(/```/g, '').trim()
    const first = cleaned.indexOf('{')
    const last = cleaned.lastIndexOf('}')
    if (first < 0 || last <= first) return params.initialDecision
    const parsed = JSON.parse(cleaned.slice(first, last + 1)) as LlmRerankResponse

    if (typeof parsed.sameDocument !== 'boolean') return params.initialDecision

    const rerankConfidence = clamp01(Number(parsed.confidence))
    const next: MatchDecision = {
