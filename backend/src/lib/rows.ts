...
let tesseractRecognizeCached: TesseractRecognizeFn | null | undefined
...
async function getTesseractRecognize(): Promise<TesseractRecognizeFn | null> {
  if (tesseractRecognizeCached !== undefined) return tesseractRecognizeCached
  const tesseractModule = (await import('tesseract.js')) as any
  tesseractRecognizeCached =
    typeof tesseractModule?.recognize === 'function'
      ? (tesseractModule.recognize as TesseractRecognizeFn)
      : typeof tesseractModule?.default?.recognize === 'function'
        ? (tesseractModule.default.recognize as TesseractRecognizeFn)
        : null
  return tesseractRecognizeCached
}

function extractVectorImageText(buffer: Buffer): string {
  const minStrLen = 4
  const out: string[] = []
  const seen = new Set<string>()
...
async function extractDocxImageOcrText(buffer: Buffer): Promise<string> {
  ...
  let recognize: TesseractRecognizeFn | null = null
  const chunks: string[] = []
  for (const name of imageNames) {
    const f = zip.files[name]
    if (!f || f.dir) continue
    const image = await f.async('nodebuffer')
    if (image.length === 0 || image.length > maxImageBytes) continue
    const lower = name.toLowerCase()
    const isVectorImage = lower.endsWith('.emf') || lower.endsWith('.wmf')
    if (isVectorImage) {
      const vectorText = extractVectorImageText(image)
      if (vectorText.length >= minTextLength) chunks.push(vectorText)
      continue
    }
    ...
    try {
      ...
      if (text.length >= minTextLength) chunks.push(text)
    } catch {
...
function buildPdfFallbackRowsFromText(text: string): ParsedRow[] {
  const out: ParsedRow[] = []
  const seen = new Set<string>()
  const raw = (text ?? '').toString()
  if (!raw) return out
...
export async function extractRowsFromFile(params: { buffer: Buffer; filename: string }): Promise<ParsedRow[]> {
  ...
  if (lower.endsWith('.docx')) {
    ...
    try {
      ...
      for (const r of guessRowsFromText(ocrText)) pushParsedRowUnique(merged, seen, r)
    } catch {
  ...
  if (lower.endsWith('.pdf')) {
    ...
    const seen = new Set<string>()
    for (const r of guessed) pushParsedRowUnique(merged, seen, r)
    for (const r of extractPackQtyPairsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractProductNameRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractPurposeRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractMaterialRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractInfectionMarkerRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractDescriptionRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractLabelValueRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractColorScaleRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractAnalyticalSensitivityRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractNeedleGaugeRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractMaterialRowsFromRows(merged)) pushParsedRowUnique(merged, seen, r)
    if (merged.length === 0) {
      for (const r of buildPdfFallbackRowsFromText(combinedText)) pushParsedRowUnique(merged, seen, r)
    }
    return merged
  }
  ...
  if (
    ...
    const merged: ParsedRow[] = []
    const seen = new Set<string>()
    for (const r of guessRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractPackQtyPairsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractProductNameRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractPurposeRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractMaterialRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractInfectionMarkerRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractDescriptionRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractLabelValueRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
    for (const r of extractColorScaleRowsFromText(text)) pushParsedRowUnique(merged, seen, r)
