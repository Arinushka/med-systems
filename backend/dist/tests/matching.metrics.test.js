...
function buildDoc(id, filename, text) {
...
function createPipeline() {
    ...
    return new MatchPipeline(config, new TestEmbeddingProvider(), new InMemoryVectorRepository(), new CandidateRetriever(lexical), lexical, new SemanticMatcher(4), new StructuralMatcher(), new ExactFieldMatcher(), new DocumentScorer(config), new MatchReranker(), new MatchExplanationBuilder());
}
test('hybrid matching quality metrics baseline', async () => {
    const pipeline = createPipeline();
    const base = buildDoc('d-base', 'base.docx', `Раздел 1. Описание
Наименование товара: Тест-набор для прокальцитонина
Артикул: PCT-100
Чувствительность: не менее 95%
Специфичность: не менее 96%
Дата: 12.05.2025`);
    const expanded = buildDoc('d-expanded', 'expanded.pdf', `Раздел 1. Описание
Наименование товара: Тест набор для определения прокальцитонина
Артикул: PCT-100
Чувствительность: не менее 95%
Специфичность: не менее 96%
Дополнительные условия хранения: +2..+8`);
    const unrelated = buildDoc('d-unrelated', 'unrelated.txt', `Наименование товара: Гематологический анализатор
Артикул: HEM-777
Чувствительность: 70%
Специфичность: 72%`);
    const paraphrased = buildDoc('d-para', 'para.txt', `Продукт: Набор реагентов PCT
Идентификатор: PCT-100
Диагностическая чувствительность от 95 процентов
Диагностическая специфичность от 96 процентов`);
    const sameNameDifferentData = buildDoc('d-same-name', 'base.docx', `Наименование товара: Тест-набор для прокальцитонина
Артикул: PCT-999
Чувствительность: не менее 80%
Специфичность: не менее 81%`);
    const ruEn = buildDoc('d-ru-en', 'ru-en.docx', `Product name: Procalcitonin test kit
Part number: PCT-100
...
Specificity: at least 96%`);
    ...
    for (const c of cases) {
        ...
        if (predicted && c.expectedMatch)
            tp++;
        else if (predicted && !c.expectedMatch)
            fp++;
        else if (!predicted && c.expectedMatch)
            fn++;
        else
