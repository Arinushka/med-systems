...
export class DefaultEmbeddingProvider {
    modelId() {
        ...
        if (provider === 'ollama')
            return `ollama:${process.env.OLLAMA_EMBEDDING_MODEL ?? 'nomic-embed-text'}`;
        if (provider === 'openai')
            ...
            for (const text of batch) {
                ...
                for (let attempt = 0; attempt <= retryCount; attempt++) {
                    ...
                    try {
                        ...
                        const emb = Array.isArray(json?.embedding)
                            ? json.embedding
                            : null;
                        if (!emb || emb.length === 0)
                            throw new Error('Ollama embeddings payload has no vector');
                        if (!emb.every((x) => Number.isFinite(Number(x))))
                            throw new Error('Ollama embeddings contains non-numeric values');
                        if (expectedDim == null)
    ...
    async embed(texts) {
        ...
        if (provider === 'ollama')
            return await this.embedWithOllama(texts);
