...
export class ExactFieldMatcher {
    score(query, candidate) {
        ...
        if (q.contractNumbers.length > 0 && c.contractNumbers.length > 0 && contractHit === 0)
            conflictingFields.push('contractNumbers');
        if (q.identifiers.length > 0 && c.identifiers.length > 0 && idHit === 0)
            conflictingFields.push('identifiers');
