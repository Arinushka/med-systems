...
function indicatorIsProductNameColumn(normalizedIndicator) {
    if (normalizedIndicator.includes('наименование товара'))
        return true;
    if (normalizedIndicator.includes('наименование изделия'))
        return true;
    if (normalizedIndicator.includes('наименование') && normalizedIndicator.includes('товар'))
        return true;
    // OCR/typo: «товра»
    if (normalizedIndicator.includes('наименование') && normalizedIndicator.includes('товра'))
        return true;
    return false;
}
/** For row-parser: allow text-only values in cells under this column (DOCX line pairing). */
export function indicatorLooksLikeProductNameColumn(indicator) {
    return indicatorIsProductNameColumn(normalizeIndicator(indicator));
}
function looksNumericOnly(valueRaw) {
 ...
 */
export function couldBeProductNameValue(valueRaw) {
    const v = (valueRaw ?? '').toString().trim();
    if (!v)
        return false;
...
function extractNarrativeNameCandidates(raw) {
    ...
    const push = (s) => {
        ...
        if (!n || n.length < 8)
            return;
        if (!/(тест|полоск|анализатор|ivd|ивд|набор)/.test(n))
            return;
        if (!seen.has(n)) {
            seen.add(n);
            out.push(n);
 ...
 */
export function looksLikeAuxValueForFirstColumnProduct(valueRaw) {
    const n = normalizeText(valueRaw ?? '');
    if (!n)
        return false;
    if (/^[0-9]+([.,][0-9]+)?$/.test(n))
        return true;
    if (/^\d{2}\.\d{2}\.\d{2}\.\d{3}-\d+$/.test((valueRaw ?? '').toString().trim()))
        return true;
    const aux = new Set([
        'ед изм',
        'единица измерения',
    ...
    if (aux.has(n))
        return true;
    if (n.length <= 14 && (n.includes('шт') || n.includes('набор') || n.includes('ед изм')))
        return true;
    return false;
}
/**
 ...
 */
export function extractNormalizedProductNamesFromRows(rows) {
    const seen = new Set();
    const out = [];
    for (const r of rows) {
        const ind = normalizeIndicator(r.indicator);
        if (!indicatorIsProductNameColumn(ind))
            continue;
        const valueRaw = (r.valueRaw ?? '').toString();
        if (!valueRaw || looksNumericOnly(valueRaw))
            continue;
        if (!couldBeProductNameValue(valueRaw))
            continue;
    ...
    for (const r of rows.slice(0, 40)) {
        ...
        if (indNorm.includes('техническое задание') || indNorm.includes('объект закупки'))
            continue;
        if (!/(ивд|тест|наркот|реагент|издел|анализ|кассет|полоск|панел|контейнер)/.test(indNorm))
            continue;
        if (!couldBeProductNameValue(indicatorRaw))
            continue;
        if (!looksLikeAuxValueForFirstColumnProduct(valueRaw))
            continue;
    ...
    for (const r of rows.slice(0, 80)) {
        ...
        if (!indicatorRaw || !valueRaw)
            continue;
        const indNorm = normalizeText(indicatorRaw);
        if (!indNorm.includes('объект закупки') &&
        ...
        for (const c of candidates) {
            ...
            if (!/(ивд|тест|наркот|панел|кассет|контейнер|индикатор|полоск)/.test(c))
                continue;
            if (!seen.has(c)) {
                seen.add(c);
                out.push(c);
...
export function productNameListsMatch(a, b) {
...
export function extractProductNameFromRows(rows) {
